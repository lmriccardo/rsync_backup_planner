...
Last Error : <last-error-from-logfile>
```
...
- Include the last error into the output.
- The last error can be retrieve from the log file
    - In order to do this You will need to modify the `run` command
      to include also the stderr into the log file output.